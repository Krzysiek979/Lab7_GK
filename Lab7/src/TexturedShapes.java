...
public class TexturedShapes {
     ...
     * in the xy-plane and its axis along the positive z-axis.
     */
    static void uvCone(GL2 gl) {
        uvCone(gl, 0.5, 1, 32, 10, 5, true);
    }

    /*
        Draws pyramid using already implemented (and slightly changed, commented it) uvCone() method.
     */
     ...
     */
    private static void uvSphere(GL2 gl, double radius, int slices, int stacks, boolean makeTexCoords) {
        if (radius <= 0)
        ...
        for (int j = 0; j < stacks; j++) {
            ...
            for (int i = 0; i <= slices; i++) {
                ...
                gl.glNormal3d(x2, y2, sinLat2);
                if (makeTexCoords)
                    gl.glTexCoord2d(1.0 / slices * i, 1.0 / stacks * (j + 1));
                gl.glVertex3d(radius * x2, radius * y2, radius * sinLat2);
                gl.glNormal3d(x1, y1, sinLat1);
                if (makeTexCoords)
                    gl.glTexCoord2d(1.0 / slices * i, 1.0 / stacks * j);
                gl.glVertex3d(radius * x1, radius * y1, radius * sinLat1);
            }
            gl.glEnd();
        }
    } // end uvSphere
     ...
     */
    private static void uvCylinder(GL2 gl, double radius, double height,
        ...
        for (int j = 0; j < stacks; j++) {
            ...
            for (int i = 0; i <= slices; i++) {
                ...
                gl.glNormal3d(x, sinLong, 0);
                if (makeTexCoords)
                    gl.glTexCoord2d(1.0 / slices * i, 1.0 / stacks * (j + 1));
                gl.glVertex3d(radius * x, radius * sinLong, z2);
                if (makeTexCoords)
                    gl.glTexCoord2d(1.0 / slices * i, 1.0 / stacks * j);
                gl.glVertex3d(radius * x, radius * sinLong, z1);
            }
            gl.glEnd();
        }
     ...
     */
    private static void uvCone(GL2 gl, double radius, double height,
                               int slices, int stacks, int rings, boolean makeTexCoords) {
        if (radius <= 0)
            throw new IllegalArgumentException("Radius must be positive.");
        if (height <= 0)
        ...
        for (int j = 0; j < stacks; j++) {
            ...
            for (int i = 0; i <= slices; i++) {
                ...
                double normLength = Math.sqrt(x * x + sinLong * sinLong + nz * nz);
                gl.glNormal3d(x / normLength, sinLong / normLength, nz / normLength);

                // Had to make adjustments so texture on both cone and pyramid is being applied normally.
                // By default it was horizontally mirrored.
                if (makeTexCoords)
                    gl.glTexCoord2d(1.0 / slices * i, /* here -> */-1.0 / stacks * (j + 1));
                gl.glVertex3d((height - z2) / height * radius * x, (height - z2) / height * radius * sinLong, z2);
                if (makeTexCoords)
                    gl.glTexCoord2d(1.0 / slices * i, /* and here -> */-1.0 / stacks * j);
                gl.glVertex3d((height - z1) / height * radius * x, (height - z1) / height * radius * sinLong, z1);
            }
            gl.glEnd();
        }
        if (rings > 0) {
     ...
     */
    private static void uvTorus(GL2 gl, double outerRadius, double innerRadius,
     ...
     */
    private static void square(GL2 gl, double side, boolean makeTexCoords) {
     ...
     */
    private static void circle(GL2 gl, double radius, int slices, int rings, boolean makeTexCoords) {
        if (radius <= 0)
     ...
     */
    private static void ring(GL2 gl, double innerRadius, double outerRadius,
                             int slices, int rings, boolean makeTexCoords) {
        if (innerRadius < 0)
